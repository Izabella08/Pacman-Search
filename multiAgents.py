...
class ReflexAgent(Agent):
    ...
    def getAction(self, gameState):
        ...
        scores = [self.evaluationFunction(gameState, action) for action in legalMoves] #pt fiecare actiune legala calculeaza scorul
        bestScore = max(scores)
        bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore] #lista cu indicii care au scor maxim
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    ________________________________
      ...
      def max-value(state):                                 def exp-value(state):
        initialize v=-inf                                       initialize v=0
    ...
    def value(self, gameState, agentIndex, nodeDepth):
        #nodeDepth =cat de adanc e arborele
        #agentIndex = indexul agentului,Pacman=0,fantomele 1,2 ....

        #daca indexul agentului e mai mare decat numarul tuturor agentilor
        if agentIndex >= gameState.getNumAgents():
            #resetam indexul agentului
            agentIndex = 0
            #crestem adancimea arborelui
            nodeDepth = nodeDepth + 1

        #daca este un nod terminal returnam functia de utilitate
        if nodeDepth == self.depth:
            return self.evaluationFunction(gameState)

        #daca urmatorul agent este de tipul MAX: apelam max_value
        if agentIndex == self.index:
            return self.max_value(gameState, agentIndex, nodeDepth)
        #daca agentul este de tip MIN: apelam exp_value
        else:
    ...
    def max_value(self, gameState, agentIndex, nodeDepth):
        #daca Pacman a castigat/pierdut returnam functia de utilitate
        if gameState.isWin() or gameState.isLose():
            return self.evaluationFunction(gameState)
        v = -999999 #incepem cu valoarea de la - infinit
        for legalActions in gameState.getLegalActions(agentIndex):   #luam actiunile posibile ale agentului
            successor = gameState.generateSuccessor(agentIndex, legalActions) #luam starea succesoare
            aux = self.value(successor, agentIndex + 1, nodeDepth)    #apelam functia value pt fiecare stare succesoare
            if aux > v: #calculam maximul dintre valori
                v = aux
    ...
    def exp_value(self, gameState, agentIndex, nodeDepth):
        if gameState.isWin() or gameState.isLose(): #daca Pacman a castigat/pierdut returnam functia de utilitate
            return self.evaluationFunction(gameState)
        v = 0 #pornim cu valoare de la 0
        probability = 1.0 / len(gameState.getLegalActions(agentIndex)) #calculam probabilitatea pentru fiecare ramura
        for legalActions in gameState.getLegalActions(agentIndex):  #luam actiunile posibile ale agentului
            successor = gameState.generateSuccessor(agentIndex, legalActions)  #luam starea succesoare
            aux = self.value(successor, agentIndex + 1, nodeDepth)   #apelam functia value pt fiecare stare succesoare
            v = v + (aux * probability)  #pentru a calcula valoarea se inmulteste probabilitatea cu valoarea fiecarei stari succesoare
            actionValue = legalActions
        return v
...
class RandomAgent (Agent):
    def getAction(self, gameState):
        ...
        chosenIndex = random.choice(range(0, len(legalMoves)))  #alege random o actiune legala
        return legalMoves[chosenIndex]
